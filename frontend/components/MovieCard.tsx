    ...
    return (
        <Link href={`/movie/${id}`} className="group flex flex-col gap-3 cursor-pointer">
            ...
            <div className="flex flex-col px-1">
                <h3 className="font-bold text-[#0A1116] leading-tight line-clamp-1 group-hover:text-[#4590BC] transition-colors">
                    {title}
                </h3>
                ...
                <div className="flex flex-wrap gap-1 mt-2">
                    {genres.slice(0, 2).map((genre) => (
                        <span
                            key={genre}
                            className="px-2 py-0.5 bg-gray-100 text-gray-600 text-[10px] uppercase font-bold tracking-wider rounded-sm"
                        >
                            {genre}
                        </span>
                    ))}
